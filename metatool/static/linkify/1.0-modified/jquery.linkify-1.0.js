...
(function($){
    ...
    httpOrMailtoUrl = /\b((?:[a-z][\w-]+:(?:\/{1,3}|[a-z0-9%])|www\d{0,3}[.]|[a-z0-9.\-]+[.][a-z]{2,4}\/)(?:[^\s()<>]+|\(([^\s()<>]+|(\([^\s()<>]+\)))*\))+(?:\(([^\s()<>]+|(\([^\s()<>]+\)))*\)|[^\s`!()\[\]{};:'".,<>?«»“”‘’]))/gi, // ET: new regex from stackoverflow, to extend links past right parenthesis ) symbol
        linkifier = function ( html ) {
            return html
                      .replace(httpOrMailtoUrl, '<a href="http://$1">$1</a>') // ET: added http:// protocol - may break all other protocols

                      // ET: commented these 2 lines out, were causing
                      // problems with no-protocol links (completely
                      // borked links generated, unclickable)
